...
class GradientCallback(TrainerCallback):
    def __init__(self, fisher, trainable_parameters, previous_weights, *args, **kwargs):
        super().__init__()
        self.fisher = fisher
        self.trainable_params = trainable_parameters
    ...
    def on_optimizer_step(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        for n, p in self.model.named_parameters():
            if n in self.trainable_params.keys() and p.grad is not None:
                self.fisher[n] += p.grad.detach().clone().data.pow(2)


class EWCTrainer(SFTTrainer):
    """
            Modify the standard loss calculation to include the regularization term.
            More details in: https://arxiv.org/pdf/1612.00796.
        """

    def __init__(self, dataset_id, ewc_lambda, fisher, trainable_parameters, previous_weights, *args, **kwargs):
        ...
        self.cb = None
        self.add_callback(GradientCallback(fisher=self.fisher, trainable_parameters=self.trainable_parameters, previous_weights=self.previous_weights, model=self.model))
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Clone the loss tensor to avoid in-place modification of a view
        loss = loss.clone() + ewc_loss
    ...
    def compute_ewc_loss(self, model):
        ...
        for n, p in model.named_parameters():
            if n in self.fisher:
                ewc_loss += (self.fisher[n] * (p - self.previous_weights[n]).pow(2)).sum() * self.ewc_lambda / 2

        if ewc_loss < 1e-5:
            Logger.instance().warning("EWC regularization loss is too small, please check the hyper-parameters")
        return ewc_loss


class EWCModelTrainer(ModelTrainer):
    def __init__(self, config):
        ...
        assert len(self.fisher) > 0 and len(self.trainable_params) > 0, Logger.instance().warning("WARNING: Fisher and previous_weights should not be empty")

    def init_fisher_and_weights(self):
        Logger.instance().debug("Initialize fisher and weights")
        for n, p in self.model.named_parameters():
            if p.requires_grad:
...
class EWC(TrainingManager):
    ...
    def __init__(self, config: dict):
        super().__init__(config)
        self.config = config
        self.trainer = EWCModelTrainer(config)
        self.router = None
    ...
    def train_ewc(self):
        ...
        Logger.instance().debug(f"EWC Training Configuration:")
        ...
        Logger.instance().debug(f"  - ewc_lambda: {self.config.get('ewc_lambda', 1.8)}")
