""" Modified from https://github.com/hyintell/CITB/blob/main/continual_learning/ni_dataset_for_cl.py """
...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, 
        ...
        **kwargs):
        ...
        self.task_split_file_name=task_split_file_name # txt file with the list of tasks
        self.load_official_test=load_official_test # to test on official test tasks
...
class NaturalInstructions(datasets.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, path=None, task_dir=None, max_num_instances_per_task=None, subset=None):
        """Yields examples."""
        Logger.instance().info(f"Generating tasks from = {path}")
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    if subset == "official_test" or subset == "test":
                        if max_num_instances_per_task is not None and max_num_instances_per_task >= 0:
                            ...
                            Logger.instance().info(f"subset: {subset}, len: {len(instances)}, max_num_instances_per_task: {max_num_instances_per_task}")
                    else:
